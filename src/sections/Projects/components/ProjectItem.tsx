...
const ProjectItem: React.FC<ProjectItemProps> = ({ projectData, num }) => {
  return (
    <div className='py-20 lg:py-40' style={{ background: projectData.background }}>
      <div className={'mx-auto flex max-w-[1360px] flex-wrap px-4'}>
        <div className='w-full text-white lg:w-1/3'>
          ...
          <Reveal>
            <p className='my-6 text-lg'>{projectData.description}</p>
          </Reveal>
          <Reveal>
          ...
          <div className='mt-12 text-center'>
            <Reveal>
              <Link variant='buttonAccent' href={projectData.link} target='_blank'>
                Смотреть проект
              </Link>
            </Reveal>
          </div>
